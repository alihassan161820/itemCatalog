...
# use session as login_Session == like a dictionary to store user's
#  sessions with our server
...
def catalogsJSON():
    ...
    cDict = {"Categories": []}
    for c in catalogs:
        cDict["Categories"].append({
            'id': c.id,
            'name': c.name,
            'item': []
        })
    for c in cDict["Categories"]:
        for i in items:
...
# get all categories and latest item
@app.route('/catalog')
@app.route('/')
def showCatalog():
    ...
    if 'username' not in login_session:
        ...
        return render_template('userLatest.html',
                               categories=categories, items=items)


# get all item belong to specific category
...
# get description paragraph of specific item
@app.route('/catalog/<string:catalog_name>/<string:item_name>')
def showItemDescription(catalog_name, item_name):
    category = session.query(Category).filter_by(name=catalog_name).one()
    cid = int(category.id)
    item = session.query(Item). \
        filter_by(name=item_name, category_id=cid).one()
    if 'username' not in login_session:
        return render_template('itemDescription.html', item=item)
    else:
        return render_template('userItemDescription.html', item=item)
...
def addItems():
    ...
    if request.method == 'POST':
        category = request.form.get('cat-select')
...
def editItem(item_id):
    ...
    if request.method == 'POST':
        ...
        editedItem.description = 'aa'
        editedItem.category_id = cid
